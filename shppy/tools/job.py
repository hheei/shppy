...
def make(
    partition: Annotated[Optional[str], typer.Option("--partition", "-p", help="Partition to submit the job to.")] = None,
    output: Annotated[str, typer.Option("--output", "-o", help="Output path for the generated job script.")] = "job.sh",
):
...
def init(
    user: Annotated[Optional[str], typer.Option("--user", "-u", help="Task running user.")] = None,
    global_log_dir: Annotated[str, typer.Option("--global-log-dir", "-g", help="Global log directory to link job logs to.")] = "~/.jobs",
):
