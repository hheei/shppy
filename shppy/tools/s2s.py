...
from shppy.tools.inputs import input_path, input_formats
...
assert set(KNOWN_FORMATS.values()) == set(DEFAULT_EXTENSION.keys()), "KNOWN_FORMATS and DEFAULT_EXTENSION keys must match."

_BUILD_FORMAT_CACHE = [(re.compile(pattern), fmt) for pattern, fmt in KNOWN_FORMATS.items()]
...
def run(
    ctx: typer.Context,
    inp: Annotated[Optional[str], typer.Option("--input", "-i", help="Input structure file path.")] = None,
    fmt: Annotated[Optional[str], typer.Option("--format", "-f", help="Input structure format (overrides auto-detection).")] = None,
    output: Annotated[Optional[str], typer.Option("--output", "-o", help="Output structure file path.")] = None,
    out_format: Annotated[Optional[str], typer.Option("--out-format", "-F", help="Output format (overrides auto-detection).")] = None,
):
    ...
    if fmt is not None and fmt not in available:
        ...
        exit(1)
    if out_format is not None and out_format not in available or out_format == 'expresso-xml':
    ...
    if output is None:
        ...
        out_pth = input_path("Output structure file", default=str(out_pth), ghost=str(out_pth), need_exist=False)
    else:
        out_pth = Path(output)
